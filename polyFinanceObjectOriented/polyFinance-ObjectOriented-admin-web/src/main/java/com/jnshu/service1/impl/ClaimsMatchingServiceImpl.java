...
//开启事务
@Transactional
public class ClaimsMatchingServiceImpl implements ClaimsMatchingService {
    @Autowired
    ...
    @Autowired
    ClaimsMatchingMapper claimsMatchingMapper;
    @Autowired
    ...
    public Claims getClaimsInfoById(long claimsId) {
        //查询债权匹配列表的data1
        return claimsMapper.getClaimsMatchingById(claimsId);
    }
    ...
    public Page<ClaimsMatchingRO> getClaimsMatchingListByRpo(ClaimsMatchingRPO rpo) {
        ...
        if (rpo.getProductName()!=null&&!rpo.getProductName().equalsIgnoreCase("")){
            rpo.setProductId(productMapper.getProductIdByProductName(rpo.getProductName()));
        }
        //如果userName不为空，就查询对应的userId[]
        if(rpo.getUserName()!=null&&!rpo.getUserName().equalsIgnoreCase("")){
            rpo.setUserId(userMapper.getUserIdByName(rpo.getUserName()));
        }
        //利用分页查询符合条件的数据
        Page<ClaimsMatchingRO> claimsMatchingROPage= PageHelper.startPage(rpo.getPage(),rpo.getSize());
        //拦截这条语句查询相关信息并分页封装
        transactionMapper.getClaimsMatchingList(rpo);
        //利用for循环遍历添加债权协议，产品名，用户名
        for (ClaimsMatchingRO ro : claimsMatchingROPage) {
            ro.setClaimsProtocolCode(contractMapper.getClaimsCodeByContractCode(ro.getContractCode()));
            ro.setProductName(productMapper.getProductNameByProductId(ro.getProductId()));
            ro.setUserName(userMapper.getUserNameById(ro.getUserId()));
        }
    ...
    public List<ContractMatchingRO> getClaimsMatchingListById(long claimsId) {
        ...
        //查询此债权已经匹配的用户
        Long[] userIds=transactionMapper.getUserIdByClaimsId(claimsId);
        //查询此债权到期时间
        long claimsLendEndAt=claimsMapper.getLendEndById(claimsId);
//        System.out.println(claimsLendEndAt);
        //查询此债权待匹配金额
        String claimsRemanentMoneyS=claimsMapper.getClaimsMatchingById(claimsId).getRemanentMoney();
        BigDecimal claimsRemanentMoney=new BigDecimal(claimsRemanentMoneyS);
//        System.out.println(claimsRemanentMoney);
        List<String> contractCodes=contractMapper.getContractCodeNotMatching();
        List<ContractMatchingRO> contractMatchingROES=new ArrayList<>();
        //查询对应信息
        for (int i=0;i<contractCodes.size();i++){
            //查询合同编号对应信息并放进roes对象中
            contractMatchingROES.add(transactionMapper.getContractInfoByContractCode(contractCodes.get(i)));
            //将ro对象中的UserName和productName放进去
            contractMatchingROES.get(i).setUserName(userMapper.getUserNameById(contractMatchingROES.get(i).getUserId()));
            contractMatchingROES.get(i).setProductName(productMapper.getProductNameByProductId(contractMatchingROES.get(i).getProductId()));
...
//            System.out.println("______________________________________");
    ...
    public int saveClaimsMatching(ClaimsMatching claimsMatching) {
        //查询交易对应的金额
        String money=transactionMapper.getContractInfoByContractCode(claimsMatching.getContractCode()).getMoney();
        //获得最新的债权协议编号
        String newestClaimsProtocolCode=claimsMatchingMapper.getNewestClaimsProtocolCode();
        ...
        //修改合同表相关信息
        int b=contractMapper.updateClaimsInfo(contract);
        ...
        //修改交易表相关信息
        int c=transactionMapper.updateClaimsId(transaction);
        System.out.println("c="+c);
        //获得原待匹配金额
        Claims claims=claimsMapper.getClaimsMatchingById(claimsMatching.getClaimsId());
        String remanentMoney=claims.getRemanentMoney();
        String lendMoney=claims.getLendMoney();
        //获得债权结束时间
        long lendEndAt=claims.getLendEndAt();
        //获得债权匹配线
        String creditorLine=systemDataMapper.getCreditorLine();
        ...
        //修改债权表待匹配金额和状态
        int d=claimsMapper.updateClaimsMoney(claims1);
        System.out.println("d="+d);
        //获得合同结束时间
        long endAt=transactionMapper.getContractInfoByContractCode(claimsMatching.getContractCode()).getEndAt();
